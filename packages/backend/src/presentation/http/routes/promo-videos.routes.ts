...
promoVideosRoutes.post('/:videoId/start-view', async (c: Context) => {
    try {
        ...
        // Verificar pré-requisitos de audiência (Filtros de Qualidade)
        ...
        // Verificar se esse dispositivo já viu esse vídeo (Anti-Fazendinha)
...
promoVideosRoutes.post('/:videoId/complete-view', async (c: Context) => {
    try {
        ...
        // Calcular Pontos
        const baseViewPoints = Math.floor(viewerEarningAmount * POINTS_PER_REAL);
        let missionPoints = 0;
        ...
        if (isBot) {
            console.error(`[SILENT-BAN] Antigravity detectou bot na conta ${userPayload.id}. Flagging user.`);
        ...
        await executeInTransaction(pool, async (client) => {
            ...
            // Pagamento Instantâneo apenas da Visualização
            await client.query('UPDATE users SET video_points = video_points + $1 WHERE id = $2', [baseViewPoints, userPayload.id]);

            // Lógica de Missão Pendente
            if (missionPoints > 0) {
                await client.query(`
            ...
            // Trigger Auditoria Automática Triple-Check (A cada 20 views)
            if (view.total_views % 20 === 0 && (view.require_like || view.require_comment || view.require_subscribe)) {
                const stats = await getYouTubeFullStats(view.video_url);
                if (stats) {
                    const pool = getDbPool(c);

                    // 1. Verificar Likes
                    const likesTarget = (await pool.query('SELECT COUNT(*) FROM promo_video_views WHERE video_id = $1 AND liked = TRUE', [videoId])).rows[0].count;
                    const likesGrowth = stats.likes - view.external_initial_likes;

                    // 2. Verificar Comentários
                    const commentsTarget = (await pool.query('SELECT COUNT(*) FROM promo_video_views WHERE video_id = $1 AND commented = TRUE', [videoId])).rows[0].count;
                    const commentsGrowth = stats.comments - view.external_initial_comments;

                    // 3. Verificar Inscritos
                    const subsTarget = (await pool.query('SELECT COUNT(*) FROM promo_video_views WHERE video_id = $1 AND subscribed = TRUE', [videoId])).rows[0].count;
                    const subsGrowth = stats.subscribers - view.external_initial_subscribers;

                    // Cálculo de Saúde Geral (Média das conversões ativas)
                    let totalWeights = 0;
                    let successScore = 0;
                    ...
                    // AÇÃO AUTOMÁTICA EM CASO DE FRAUDE (Health < 40%)
                    let statusUpdate = view.status;
                    let rejectionReason = null;

                    if (finalHealth < 40) {
                        statusUpdate = 'PAUSED';
                        rejectionReason = 'SUSPEITA DE FRAUDE: Baixa taxa de engajamento real detectada pelo Auditor Automático.';
                        console.error(`[ANTI-FRAUDE] Campanha ${videoId} pausada automaticamente. Health: ${finalHealth}%`);
                    }
                    ...
                    // LIBERAÇÃO AUTOMÁTICA DE PONTOS SE SAÚDE FOR BOA (> 70%)
                    if (finalHealth >= 70) {
                    ...
                    if (finalHealth < 50) {
                        console.warn(`[AUDITOR-DETECTION] Campanha ${videoId} sinalizada com baixa qualidade: Health ${finalHealth}%`);
                    }
                }
            }
        });
