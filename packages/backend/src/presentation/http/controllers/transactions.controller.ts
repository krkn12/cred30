...
export class TransactionsController {
    ...
    static async withdraw(c: Context) {
        try {
            ...
            if (amount > 10000) {
                return c.json({ success: false, message: 'Valor máximo por saque é R$ 10.000,00' }, 400);
            }
            ...
            if (!userCheck.rows[0]?.cpf) {
                return c.json({
                    success: false,
                    message: 'CPF obrigatório para realizar saques.',
                    code: 'CPF_REQUIRED'
                }, 400);
            }
           ...
           VALUES ($1, $2, $3, $4, $5, $6)`,
                    [
                        user.id,
                        '⏳ Saque Solicitado',
                        `Seu saque de R$ ${netAmount.toFixed(2)} foi registrado. O PIX será enviado em até 24h úteis.`,
                        'WITHDRAWAL_PENDING',
    ...
    static async reviewTransaction(c: Context) {
        try {
            ...
            if (txCheck.rows.length === 0) {
                return c.json({ success: false, message: 'Transação não encontrada' }, 404);
            }
    ...
    /**
     * Listar avaliações públicas aprovadas
     */
