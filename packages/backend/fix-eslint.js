...
for (const file of allFiles) {
    ...
    if (file.endsWith('setup.ts')) {
        content = content.replace(/process\.env\.JWT_SECRET\s*=\s*process\.env\.JWT_SECRET;?/g, '');
    }

    // 2. Fix empty catch blocks and unused catch vars
    // empty catch(e){} -> catch(e) { console.error(e); }
    content = content.replace(/catch\s*\(\s*([a-zA-Z0-9_]+)\s*(:\s*any|\s*:\s*unknown)?\s*\)\s*\{\s*\}/g, 'catch ($1$2) {\n      console.error($1);\n    }');
    // empty catch { } -> catch (error) { console.error(error); }
    content = content.replace(/catch\s*\{\s*\}/g, 'catch (error) {\n      console.error(error);\n    }');

    // 3. Fix `error: any` in catch
    // In TS, catch variables can only be any or unknown.
