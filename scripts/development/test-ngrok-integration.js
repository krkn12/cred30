/**
 * Script de teste para validar a integração do CRED30 com ngrok
 * Este script verifica se todos os endpoints estão funcionando corretamente
 * através das URLs públicas do ngrok
 */

const axios = require('axios');

// Configurações (serão obtidas dos argumentos ou variáveis de ambiente)
const config = {
...
async function testConnectivity() {
  ...
  try {
    // Testar backend
    logInfo('Testando acesso ao backend...');
    const backendResponse = await axios.get(`${config.backendUrl}/api/health`, { timeout: config.timeout });
    if (backendResponse.status === 200) {
      logSuccess(`Backend acessível: ${config.backendUrl}`);
    }
  } catch (error) {
    logError(`Backend inacessível: ${config.backendUrl}`);
    throw error;
  }
}
...
async function testAuthentication() {
  logHeader('Testando Autenticação');
  ...
  try {
    ...
    const adminResponse = await axios.post(`${config.backendUrl}/api/auth/login`, credentials.admin, { timeout: config.timeout });
    
    if (adminResponse.data && adminResponse.data.token) {
      adminToken = adminResponse.data.token;
      logSuccess('Login admin realizado com sucesso');
    } else {
  ...
  try {
    ...
    const clientResponse = await axios.post(`${config.backendUrl}/api/auth/login`, credentials.client, { timeout: config.timeout });
    
    if (clientResponse.data && clientResponse.data.token) {
      clientToken = clientResponse.data.token;
      logSuccess('Login cliente realizado com sucesso');
    } else {
...
async function testMainEndpoints(tokens) {
  logHeader('Testando Endpoints Principais');
...
async function testFunctionalities() {
  logHeader('Testando Funcionalidades Específicas');
  ...
  try {
    ...
    const loginResponse = await axios.post(`${config.backendUrl}/api/auth/login`, credentials.client);
    const token = loginResponse.data.token;
    
    const loanResponse = await axios.post(
      `${config.backendUrl}/api/loans`,
      loanData,
      { 
    ...
    if (loanResponse.status === 201) {
      logSuccess('Criação de empréstimo: OK');
    }
  } catch (error) {
  ...
  try {
    ...
    const withdrawalResponse = await axios.post(
      `${config.backendUrl}/api/withdrawals`,
      withdrawalData,
      { 
        headers: { 'Authorization': `Bearer ${token}` },
        timeout: config.timeout
    ...
    if (withdrawalResponse.status === 201) {
      logSuccess('Requisição de saque: OK');
    }
  } catch (error) {
...
async function runTests() {
  log('\n🚀 Iniciando Testes de Integração CRED30 com ngrok');
  log('=' .repeat(60));
  ...
  try {
    ...
    logInfo('A plataforma CRED30 está funcionando corretamente com ngrok.');
    
  } catch (error) {
    log('\n' + '='.repeat(60));
    logError('❌ Testes falharam!');
    logError(`Erro: ${error.message}`);
    logWarning('Verifique os logs dos containers e a configuração do ngrok.');
    process.exit(1);
  }
}
...
function showHelp() {
  log('\n📖 Como usar:');
  log('   node test-ngrok-integration.js');
  log('   FRONTEND_URL=https://abc-123.ngrok-free.app BACKEND_URL=https://def-456.ngrok-free.app node test-ngrok-integration.js');
  log('');
  log('🔧 Variáveis de ambiente:');
  log('   FRONTEND_URL: URL pública do frontend (padrão: http://localhost:5173)');
  log('   BACKEND_URL: URL pública do backend (padrão: http://localhost:3001)');
  log('');
}
