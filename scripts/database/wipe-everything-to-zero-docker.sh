...
# =============================================================================
# SCRIPT DE LIMPEZA COMPLETA - DEIXAR TUDO ZERADO - DOCKER
# =============================================================================
...
DOCKER_USER=${DOCKER_USER:-cred30user}
...
if docker exec "$DOCKER_CONTAINER" pg_dump -U "$DOCKER_USER" -d "$DOCKER_DB" --verbose --clean --no-acl --no-owner > "$BACKUP_FILE"; then
    ...
    show_warning "Falha ao criar backup de segurança!"
    exit 1
fi
...
show_step "Verificando que tudo foi apagado..."
...
show_success "BANCO DE DADOS 100% LIMPO E ZERADO!"
echo -e "${GREEN}🎯 RESULTADO FINAL:${NC}"
echo -e "${GREEN}✅ BANCO COMPLETAMENTE VAZIO${NC}"
echo -e "${GREEN}✅ TODOS OS DADOS APAGADOS${NC}"
echo -e "${GREEN}✅ TODAS AS SEQUÊNCIAS RESETADAS${NC}"
...
echo -e "${RED}O sistema está completamente vazio e precisará ser reconfigurado!${NC}"
