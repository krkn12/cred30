...
async function cleanupUsers() {
    try {
        ...
        const result = await pool_1.pool.query("SELECT id, name, email FROM users ORDER BY name");
...
async function deleteUserRecursively(userId) {
    try {
        // Safe deletion order (children first)
        // 1. Logs & History
        await pool_1.pool.query('DELETE FROM rate_limit_logs WHERE user_id = $1', [userId]);
        await pool_1.pool.query('DELETE FROM admin_logs WHERE admin_id = $1', [userId]);
        // 2. Financials & Loans
        ...
        `, [userId]);
        await pool_1.pool.query('DELETE FROM loans WHERE user_id = $1', [userId]);
        ...
        await pool_1.pool.query('DELETE FROM marketplace_orders WHERE buyer_id = $1 OR seller_id = $1 OR courier_id = $1', [userId]);
        // Delete listings by seller
        await pool_1.pool.query('DELETE FROM marketplace_listings WHERE seller_id = $1', [userId]);
        // 4. Assets & Core
        await pool_1.pool.query('DELETE FROM quotas WHERE user_id = $1', [userId]);
        await pool_1.pool.query('DELETE FROM referral_codes WHERE created_by = $1', [userId]);
        // 5. The User
